  ...
  return(
    <div style={{background:T.surface,border:"1px solid "+T.border,borderRadius:12,padding:16,cursor:"pointer",transition:"box-shadow 0.1s"}} onClick={p.onClick}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:10,paddingTop:8,borderTop:"1px solid "+T.border}}>
        <div style={{fontSize:10,color:T.muted}}>{s.gestionnaire||"â"}</div>
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
        <div style={{background:T.surface,border:"1px solid "+T.border,borderRadius:10,padding:14}}>
          ...
          {[
            {l:"President",v:s.president||"â"},
            {l:"Courriel",v:s.courriel||"â"},
            {l:"Telephone",v:s.tel||"â"},
            {l:"Immatriculation",v:s.immat||"â"},
            {l:"Annee construction",v:s.anneeConstruction||"â"},
            {l:"Exercice financier",v:s.exercice||"â"},
            {l:"Gestionnaire Predictek",v:s.gestionnaire||"â"},
    ...
    <div style={{maxWidth:600,margin:"0 auto"}}>
      ...
      {etape===2&&(
        <div>
          ...
          <div style={{fontSize:12,color:T.muted,marginBottom:16}}>Importez votre fichier Excel/CSV avec les informations des coproprietaires. Ce n est pas obligatoire â vous pouvez les ajouter manuellement plus tard.</div>
          ...
          <div style={{background:T.accentL,border:"1px solid "+T.accent+"44",borderRadius:10,padding:16,marginBottom:16}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {[
                ...
                {l:"President",v:form.president||"â"},{l:"Immatriculation",v:form.immat||"â"},
                {l:"Exercice",v:form.exercice},{l:"Gestionnaire",v:form.gestionnaire||"â"},
                {l:"Adresse",v:form.adr||"â"},{l:"Coproprietaires CSV",v:nbImport>0?nbImport+" detectes":"A ajouter manuellement"},
              ].map(function(item,i){return(
                <div key={i}>
          ...
          <tbody>
            {usagers.map(function(u){return(
              <tr key={u.id} style={{borderBottom:"1px solid "+T.border,background:u.actif?T.surface:T.alt}}>
                ...
                <td style={{padding:"9px 12px",fontSize:11,color:T.muted}}>{u.derniereConnexion||"â"}</td>
                <td style={{padding:"9px 12px"}}>
...
var PARAMS_DEFAUT={
  ...
  courrielCopropriÃ©taires:"info@syndicatpiedmont.com",
  ...
  documents:[
    ...
    {id:3,nom:"Reglement 2024-001 â Animaux",type:"reglement",date:"2024-03-20",taille:"120 KB",dispo:true},
    {id:4,nom:"Reglement 2024-002 â Stationnement",type:"reglement",date:"2024-06-15",taille:"95 KB",dispo:false},
  ],
};
  ...
  return(
    <div style={{padding:20,fontFamily:"Georgia,serif",maxWidth:860,margin:"0 auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <div>
          <div style={{fontSize:18,fontWeight:800,color:T.navy}}>Parametres â {params.nom}</div>
      ...
      {ong==="ca"&&(
        <div>
          <Card>
            ...
            <CardSub>Selon la declaration de copropriete â le nombre de membres doit etre impair (3, 5, 7 ou 9)</CardSub>
          ...
          <Card>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr",gap:12}}>
              {[
                ...
                {k:"courrielCopropriÃ©taires",l:"Courriel communications coproprietaires",ph:"info@syndicat.com",desc:"Adresse de contact general pour les coproprietaires"},
          ...
          <Card>
            ...
            <div style={{display:"grid",gap:0}}>
              {[
                ...
                {k:"autoAlertesConformite",l:"Alertes conformite automatiques",desc:"CE, assurance, PAP â 90 jours, 30 jours, 7 jours avant expiration"},
              ].map(function(item,i){return(
...
function StepIndicator(p){
  var STEPS=["Syndicat","CA","CopropriÃ©taires","Soldes","Documents","Carnet","Attestation","Confirmation"];
  return(
...
var COMPOSANTES_LOI16=[
  ...
  {cat:"Enveloppe",nom:"Toiture â membrane et structure",dureeVie:25,anneeInstall:"",etat:"bon",notes:"",obligatoire:true},
  {cat:"Enveloppe",nom:"Fenetres et portes exterieures â parties communes",dureeVie:30,anneeInstall:"",etat:"bon",notes:"",obligatoire:true},
  {cat:"Enveloppe",nom:"Revetement exterieur",dureeVie:30,anneeInstall:"",etat:"bon",notes:"",obligatoire:true},
  {cat:"Mecanique",nom:"Systeme de chauffage â parties communes",dureeVie:20,anneeInstall:"",etat:"bon",notes:"",obligatoire:true},
  {cat:"Mecanique",nom:"Systeme de ventilation â parties communes",dureeVie:20,anneeInstall:"",etat:"bon",notes:"",obligatoire:true},
  {cat:"Mecanique",nom:"Tuyauterie et plomberie â parties communes",dureeVie:40,anneeInstall:"",etat:"bon",notes:"",obligatoire:true},
  {cat:"Mecanique",nom:"Systeme electrique â parties communes",dureeVie:40,anneeInstall:"",etat:"bon",notes:"",obligatoire:true},
  ...
  {cat:"Amenagements",nom:"Stationnement â surface et structure",dureeVie:20,anneeInstall:"",etat:"bon",notes:"",obligatoire:false},
  {cat:"Amenagements",nom:"Paysagement et amenagements exterieurs",dureeVie:20,anneeInstall:"",etat:"bon",notes:"",obligatoire:false},
  {cat:"Amenagements",nom:"Eclairage â parties communes",dureeVie:15,anneeInstall:"",etat:"bon",notes:"",obligatoire:true},
  {cat:"Interieur",nom:"Corridors et hall d entree",dureeVie:20,anneeInstall:"",etat:"bon",notes:"",obligatoire:true},
  {cat:"Interieur",nom:"Peinture â parties communes",dureeVie:10,anneeInstall:"",etat:"bon",notes:"",obligatoire:false},
];
  ...
  return(
    <div style={{padding:20,fontFamily:"Georgia,serif",maxWidth:900,margin:"0 auto"}}>
      <div style={{background:T.navy,color:"#fff",borderRadius:12,padding:"16px 20px",marginBottom:20,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div>
          <div style={{fontSize:18,fontWeight:800}}>Nouveau syndicat â Configuration initiale</div>
      ...
      {step===1&&(
        <div>
          <div style={{fontSize:15,fontWeight:700,color:T.navy,marginBottom:4}}>Etape 1 â Informations du syndicat</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            <Field l="Numero immatriculation REQ" hint="11 chiffres â registre entreprises Quebec"><input value={data.immat} onChange={function(e){sd("immat",e.target.value);}} style={INP} placeholder="1144524577"/></Field>
          ...
          <div style={{display:"flex",justifyContent:"flex-end",marginTop:20}}>
            <Btn dis={!data.nom||!data.code||!data.ville} onClick={function(){setStep(2);}}>Continuer â</Btn>
          </div>
        </div>
      )}
      ...
      {step===2&&(
        <div>
          <div style={{fontSize:15,fontWeight:700,color:T.navy,marginBottom:4}}>Etape 2 â Conseil d administration</div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:20}}>
            <Btn bg={T.alt} tc={T.muted} bdr={"1px solid "+T.border} onClick={function(){setStep(1);}}>â Retour</Btn>
            <Btn dis={!data.president} onClick={function(){setStep(3);}}>Continuer â</Btn>
          </div>
        </div>
      )}
      ...
      {step===3&&(
        <div>
          <div style={{fontSize:15,fontWeight:700,color:T.navy,marginBottom:4}}>Etape 3 â Import des coproprietaires</div>
          ...
          {csvErrors.length>0&&(
            <div style={{background:T.amberL,borderRadius:8,padding:"9px 13px",fontSize:11,color:T.amber,marginBottom:10}}>
              {csvErrors.slice(0,5).map(function(e,i){return <div key={i}>â¢ {e}</div>;})}
            </div>
          )}
          ...
          {copros.length===0&&(
            <div style={{marginBottom:14}}>
              <Lbl l="OU â Saisir le nombre d unites manuellement"/>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:20}}>
            <Btn bg={T.alt} tc={T.muted} bdr={"1px solid "+T.border} onClick={function(){setStep(2);}}>â Retour</Btn>
            <Btn dis={copros.length===0&&!data.nbUnites} onClick={function(){setStep(4);}}>Continuer â</Btn>
          </div>
        </div>
      )}
      ...
      {step===4&&(
        <div>
          <div style={{fontSize:15,fontWeight:700,color:T.navy,marginBottom:4}}>Etape 4 â Soldes d ouverture et budget</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
            ...
            <div/>
            <Field l="Solde â Compte d exploitation ($)" hint="Argent disponible pour les operations courantes"><input type="number" value={data.soldeOp} onChange={function(e){sd("soldeOp",e.target.value);}} style={INP} placeholder="Ex: 7361.88" step="0.01"/></Field>
            <Field l="Solde â Fonds de prevoyance ($)" hint="Reserve pour travaux majeurs (Loi 16 â obligatoire)"><input type="number" value={data.soldePrev} onChange={function(e){sd("soldePrev",e.target.value);}} style={INP} placeholder="Ex: 64235.01" step="0.01"/></Field>
            <Field l="Solde â Fonds d assurance ($)" hint="Reserve pour la franchise d assurance"><input type="number" value={data.soldeAss} onChange={function(e){sd("soldeAss",e.target.value);}} style={INP} placeholder="Ex: 36178.37" step="0.01"/></Field>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:20}}>
            <Btn bg={T.alt} tc={T.muted} bdr={"1px solid "+T.border} onClick={function(){setStep(3);}}>â Retour</Btn>
            <Btn onClick={function(){setStep(5);}}>Continuer â</Btn>
          </div>
        </div>
      )}
      ...
      {step===5&&(
        <div>
          <div style={{fontSize:15,fontWeight:700,color:T.navy,marginBottom:4}}>Etape 5 â Documents officiels</div>
              ...
              return(
                <div key={dtype.cat} style={{background:T.surface,border:"1px solid "+(uploaded.length>0?T.accent:dtype.obligatoire?T.amber:T.border),borderRadius:10,padding:12}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                    ...
                    {uploaded.length>0&&<span style={{fontSize:16,color:T.accent}}>â</span>}
                  </div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:20}}>
            <Btn bg={T.alt} tc={T.muted} bdr={"1px solid "+T.border} onClick={function(){setStep(4);}}>â Retour</Btn>
            <Btn onClick={function(){setStep(6);}}>Continuer â</Btn>
          </div>
        </div>
      )}
      ...
      {step===6&&(
        <div>
          <div style={{fontSize:15,fontWeight:700,color:T.navy,marginBottom:4}}>Etape 6 â Carnet d entretien (Loi 16)</div>
          ...
          <div style={{background:T.amberL,border:"1px solid "+T.amber+"44",borderRadius:8,padding:"9px 14px",marginBottom:14,fontSize:11,color:T.amber}}>
            <b>Loi 16 â Article 1070.2 CCQ:</b> Tout syndicat doit tenir un carnet d entretien de l immeuble incluant toutes les composantes majeures avec leur duree de vie prevue et leur etat actuel.
          </div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:20}}>
            <Btn bg={T.alt} tc={T.muted} bdr={"1px solid "+T.border} onClick={function(){setStep(5);}}>â Retour</Btn>
            <Btn onClick={function(){setStep(7);}}>Continuer â</Btn>
          </div>
        </div>
      )}
      ...
      {step===7&&(
        <div>
          <div style={{fontSize:15,fontWeight:700,color:T.navy,marginBottom:4}}>Etape 7 â Attestation de copropriete</div>
            ...
            <div style={{display:"grid",gap:8,marginBottom:14}}>
              {[
                ...
                <div key={i} style={{display:"flex",gap:10,alignItems:"flex-start",fontSize:11,color:T.text}}>
                  <span style={{color:T.accent,fontWeight:700,flexShrink:0,marginTop:1}}>â</span>
                  <span>{item}</span>
                </div>
              );})}
            </div>
            ...
            <div style={{borderTop:"1px solid "+T.border,paddingTop:14,marginTop:14}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,fontSize:11,color:T.text}}>
                ...
                <div>
                  ...
                  <div style={{color:T.muted}}>{today()} â {data.ville||"[Ville]"}</div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:20}}>
            <Btn bg={T.alt} tc={T.muted} bdr={"1px solid "+T.border} onClick={function(){setStep(6);}}>â Retour</Btn>
            <Btn dis={!data.attestationAcceptee} onClick={function(){setStep(8);}}>Continuer â</Btn>
          </div>
        </div>
      )}
      ...
      {step===8&&(
        <div>
          <div style={{fontSize:15,fontWeight:700,color:T.navy,marginBottom:4}}>Etape 8 â Confirmation et activation</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:20}}>
            {[
              {titre:"Syndicat",items:[{l:"Nom",v:data.nom},{l:"Code",v:data.code},{l:"Immatriculation",v:data.immat||"â"},{l:"Construction",v:data.anneeConstruction},{l:"Exercice",v:data.exercice}]},
              {titre:"CA",items:[{l:"Membres",v:data.nbMembresCA+" membres"},{l:"President",v:data.president||"â"},{l:"Secretaire",v:data.secretaire||"â"},{l:"Tresorier",v:data.tresorier||"â"}]},
              {titre:"CopropriÃ©taires",items:[{l:"Importes",v:copros.length||data.nbUnites||"0"},{l:"Cotisations/mois",v:totalCot>0?money(totalCot):"â"},{l:"Fraction totale",v:totalFraction>0?totalFraction.toFixed(3)+"%":"â"}]},
              {titre:"Finances",items:[{l:"Exploitation",v:money(parseFloat(data.soldeOp)||0)},{l:"Prevoyance",v:money(parseFloat(data.soldePrev)||0)},{l:"Assurance",v:money(parseFloat(data.soldeAss)||0)},{l:"Budget annuel",v:data.budgetAnnuel?money(parseFloat(data.budgetAnnuel)):"â"}]},
              {titre:"Documents",items:[{l:"Importes",v:data.documents.length+" document(s)"},{l:"Declaration",v:data.documents.find(function(d){return d.cat==="declaration";})?"â Presente":"â  Manquante"},{l:"Reglement",v:data.documents.find(function(d){return d.cat==="reglement";})?"â Present":"â"}]},
              {titre:"Carnet Loi 16",items:[{l:"Composantes",v:data.composantes.length+" total"},{l:"Completees",v:compOk+"/"+compOblig+" obligatoires"},{l:"Inspecteur",v:data.inspecteur||"â"},{l:"Date inspection",v:data.dateInspection||"â"}]},
          ...
          <div style={{display:"flex",justifyContent:"space-between"}}>
            <Btn bg={T.alt} tc={T.muted} bdr={"1px solid "+T.border} onClick={function(){setStep(7);}}>â Retour</Btn>
...
// ===== PARAMETRES PREDICTEK =====
function ParamsPredictek(){
  var s0=useState(function(){
    try{var s=localStorage.getItem("predictek_infos_entreprise");if(s)return JSON.parse(s);}catch(e){}
    return {
      nomLegal:"",nomCommercial:"Predictek",
      adr:"",ville:"",province:"QC",codePostal:"",
      tel:"",courriel:"",siteWeb:"",
      neq:"",
      noTPS:"",noDeclarantTPS:"",
      noTVQ:"",noDeclarantTVQ:"",
      exerciceDebut:"2025-11-01",exerciceFin:"2026-10-31",
      institutionBanque:"",noCompte:"",noTransit:"",noInstitution:"",
      courrielRemiseARC:"",courrielRemiseRQ:"",
      logo:null,
    };
  });
  ...
  function sauvegarder(){
    try{localStorage.setItem("predictek_infos_entreprise",JSON.stringify(info));}catch(e){}
    setSavedMsg("Sauvegardes!");setTimeout(function(){setSavedMsg("");},3000);
  }
  function handleLogo(e){
    var file=e.target.files[0];if(!file)return;
    var reader=new FileReader();
    reader.onload=function(ev){
      si("logo",ev.target.result);
      try{localStorage.setItem("predictek_logo",ev.target.result);}catch(e){}
    };
    reader.readAsDataURL(file);
  }
  function resetLogo(){si("logo",null);try{localStorage.removeItem("predictek_logo");}catch(e){}}
  var TABS_P=[{id:"entreprise",l:"Informations entreprise"},{id:"taxes",l:"TPS / TVQ"},{id:"banque",l:"Coordonnees bancaires"},{id:"logo",l:"Logo et apparence"}];
  var INP_S={width:"100%",border:"1px solid #DDD9CF",borderRadius:7,padding:"7px 10px",fontSize:12,fontFamily:"inherit",background:"#FFF",outline:"none",boxSizing:"border-box"};
  function Lbl2(p){return <div style={{fontSize:10,color:T.muted,textTransform:"uppercase",letterSpacing:"0.07em",fontWeight:600,marginBottom:5}}>{p.l}</div>;}
  function F(p){return <div style={p.full?{gridColumn:"1/-1"}:{}}><Lbl2 l={p.l}/>{p.children}{p.hint&&<div style={{fontSize:10,color:T.muted,marginTop:3}}>{p.hint}</div>}</div>;}
  return(
    <div style={{padding:20}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <div>
          <div style={{fontSize:16,fontWeight:800,color:T.navy}}>Parametres Predictek</div>
          <div style={{fontSize:11,color:T.muted}}>Informations legales, fiscales et configuration de l entreprise</div>
        </div>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
      ...
      <div style={{display:"flex",gap:3,marginBottom:16,background:T.surface,padding:4,borderRadius:10,border:"1px solid "+T.border}}>
        {TABS_P.map(function(t){var a=ong===t.id;return(
          <button key={t.id} onClick={function(){setOng(t.id);}} style={{background:a?T.navy:"transparent",border:"none",borderRadius:7,padding:"7px 14px",color:a?"#fff":T.muted,fontSize:12,cursor:"pointer",fontFamily:"inherit",fontWeight:a?600:400,whiteSpace:"nowrap"}}>{t.l}</button>
        );})}
      </div>
      {ong==="entreprise"&&(
        <div style={{background:T.surface,border:"1px solid "+T.border,borderRadius:12,padding:20}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <F l="Nom legal complet" full><input value={info.nomLegal} onChange={function(e){si("nomLegal",e.target.value);}} style={INP_S} placeholder="9999999 Canada Inc."/></F>
            <F l="Nom commercial" full><input value={info.nomCommercial} onChange={function(e){si("nomCommercial",e.target.value);}} style={INP_S} placeholder="Predictek"/></F>
            <F l="Adresse" full><input value={info.adr} onChange={function(e){si("adr",e.target.value);}} style={INP_S} placeholder="123 rue Principale"/></F>
            <F l="Ville"><input value={info.ville} onChange={function(e){si("ville",e.target.value);}} style={INP_S} placeholder="Quebec"/></F>
            <F l="Province"><select value={info.province} onChange={function(e){si("province",e.target.value);}} style={INP_S}><option>QC</option><option>ON</option><option>BC</option><option>AB</option></select></F>
            <F l="Code postal"><input value={info.codePostal} onChange={function(e){si("codePostal",e.target.value.toUpperCase());}} style={INP_S} placeholder="G1K 1A1"/></F>
            <F l="Telephone"><input value={info.tel} onChange={function(e){si("tel",e.target.value);}} style={INP_S} placeholder="418-555-0000"/></F>
            <F l="Courriel principal"><input value={info.courriel} onChange={function(e){si("courriel",e.target.value);}} style={INP_S} placeholder="info@predictek.ca"/></F>
            <F l="Site web"><input value={info.siteWeb} onChange={function(e){si("siteWeb",e.target.value);}} style={INP_S} placeholder="www.predictek.ca"/></F>
            <F l="NEQ (Registre entreprises Quebec)" hint="Numero d entreprise Quebec — 10 chiffres"><input value={info.neq} onChange={function(e){si("neq",e.target.value);}} style={INP_S} placeholder="1234567890"/></F>
            <F l="Debut exercice financier"><input type="date" value={info.exerciceDebut} onChange={function(e){si("exerciceDebut",e.target.value);}} style={INP_S}/></F>
            <F l="Fin exercice financier"><input type="date" value={info.exerciceFin} onChange={function(e){si("exerciceFin",e.target.value);}} style={INP_S}/></F>
          </div>
        </div>
      )}
      {ong==="taxes"&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          <div style={{background:T.surface,border:"1px solid "+T.border,borderRadius:12,padding:20}}>
            ...
            <div style={{fontSize:11,color:T.muted,marginBottom:16}}>Taxe sur les produits et services (5%)</div>
            <div style={{display:"grid",gap:10}}>
              <F l="Numero de TPS" hint="Format: 123456789 RT0001"><input value={info.noTPS} onChange={function(e){si("noTPS",e.target.value.toUpperCase());}} style={INP_S} placeholder="123456789 RT0001"/></F>
              <F l="Numero de declarant"><input value={info.noDeclarantTPS} onChange={function(e){si("noDeclarantTPS",e.target.value);}} style={INP_S} placeholder="0001"/></F>
              <F l="Courriel remises ARC"><input value={info.courrielRemiseARC} onChange={function(e){si("courrielRemiseARC",e.target.value);}} style={INP_S} placeholder="tps@predictek.ca"/></F>
            </div>
            <div style={{marginTop:12,background:T.blueL,borderRadius:8,padding:"9px 12px",fontSize:11,color:T.blue}}>
              Ces numeros apparaissent automatiquement sur vos factures PDF et rapports TPS.
            </div>
          </div>
          <div style={{background:T.surface,border:"1px solid "+T.border,borderRadius:12,padding:20}}>
            <div style={{fontSize:13,fontWeight:700,color:T.navy,marginBottom:4}}>TVQ — Revenu Quebec</div>
            <div style={{fontSize:11,color:T.muted,marginBottom:16}}>Taxe de vente du Quebec (9.975%)</div>
            <div style={{display:"grid",gap:10}}>
              <F l="Numero de TVQ" hint="Format: 1234567890 TQ0001"><input value={info.noTVQ} onChange={function(e){si("noTVQ",e.target.value.toUpperCase());}} style={INP_S} placeholder="1234567890 TQ0001"/></F>
              <F l="Numero de declarant"><input value={info.noDeclarantTVQ} onChange={function(e){si("noDeclarantTVQ",e.target.value);}} style={INP_S} placeholder="0001"/></F>
              <F l="Courriel remises Revenu QC"><input value={info.courrielRemiseRQ} onChange={function(e){si("courrielRemiseRQ",e.target.value);}} style={INP_S} placeholder="tvq@predictek.ca"/></F>
            </div>
            <div style={{marginTop:12,background:T.blueL,borderRadius:8,padding:"9px 12px",fontSize:11,color:T.blue}}>
              Ces numeros apparaissent automatiquement sur vos factures PDF et rapports TVQ.
            </div>
          </div>
          <div style={{gridColumn:"1/-1",background:T.amberL,border:"1px solid #B8602044",borderRadius:10,padding:14}}>
            <div style={{fontSize:12,fontWeight:700,color:T.amber,marginBottom:6}}>Rappel — Frequence de remise</div>
            <div style={{fontSize:11,color:T.amber,lineHeight:1.8}}>
              Mensuelle si revenus annuels TPS superieur a 1 500 000$<br/>
              Trimestrielle si revenus entre 30 000$ et 1 500 000$ (cas typique Predictek)<br/>
              Annuelle si revenus inferieurs a 30 000$
            </div>
          </div>
        </div>
      )}
      {ong==="banque"&&(
        <div style={{background:T.surface,border:"1px solid "+T.border,borderRadius:12,padding:20}}>
          ...
          <div style={{fontSize:11,color:T.muted,marginBottom:16}}>Utilisees pour les remises DAS, TPS/TVQ et les virements</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            <F l="Institution financiere" full><input value={info.institutionBanque} onChange={function(e){si("institutionBanque",e.target.value);}} style={INP_S} placeholder="Desjardins / TD / BNC..."/></F>
            <F l="Numero de transit" hint="5 chiffres"><input value={info.noTransit} onChange={function(e){si("noTransit",e.target.value);}} style={INP_S} placeholder="00000"/></F>
            <F l="Numero d institution" hint="3 chiffres"><input value={info.noInstitution} onChange={function(e){si("noInstitution",e.target.value);}} style={INP_S} placeholder="000"/></F>
            <F l="Numero de compte" full><input value={info.noCompte} onChange={function(e){si("noCompte",e.target.value);}} style={INP_S} placeholder="0000000000"/></F>
          </div>
          <div style={{marginTop:14,background:T.redL,borderRadius:8,padding:"9px 12px",fontSize:11,color:T.red}}>
            Ces informations sont sauvegardees localement dans votre navigateur. Ne partagez pas ce code source.
          </div>
        </div>
      )}
      {ong==="logo"&&(
        <div style={{background:T.surface,border:"1px solid "+T.border,borderRadius:12,padding:20}}>
          ...
          <div style={{display:"flex",gap:24,alignItems:"flex-start"}}>
            <div style={{flexShrink:0}}>
              ...
              <div style={{fontSize:10,color:T.muted,marginTop:6,textAlign:"center"}}>{info.logo?"Logo personnalise":"Logo par defaut"}</div>
            </div>
            <div style={{flex:1}}>
              <input type="file" accept="image/*" id="logoInputHub" onChange={handleLogo} style={{display:"none"}}/>
              <div style={{display:"grid",gap:10}}>
                <button onClick={function(){document.getElementById("logoInputHub").click();}} style={{background:T.accent,color:"#fff",border:"none",borderRadius:8,padding:"12px 20px",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",textAlign:"left",display:"flex",alignItems:"center",gap:10}}>
                  <span style={{fontSize:20,lineHeight:1}}>+</span>
                  <div><div>Choisir un logo</div><div style={{fontSize:10,fontWeight:400,opacity:0.8}}>PNG, JPG ou SVG — recommande 200x200px</div></div>
                </button>
                {info.logo&&<button onClick={resetLogo} style={{background:"#FDECEA",color:"#B83232",border:"1px solid #B83232",borderRadius:8,padding:"10px 20px",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>Retirer le logo</button>}
              </div>
              {info.logo?<div style={{marginTop:12,background:T.accentL,borderRadius:8,padding:"10px 14px",fontSize:11,color:T.accent}}>Logo actif — visible dans la barre de navigation.</div>:<div style={{marginTop:12,background:T.blueL,borderRadius:8,padding:"10px 14px",fontSize:11,color:T.blue}}>Cliquez pour ajouter votre logo. Il s affichera dans la barre de navigation.</div>}
            </div>
          </div>
        </div>
      )}
    ...
    return(
      <div style={{fontFamily:"Georgia,serif"}}>
        <div style={{background:T.navy,display:"flex",alignItems:"center",gap:12,padding:"12px 20px"}}>
          ...
          <span style={{fontSize:13,fontWeight:700,color:"#fff"}}>{selSP?selSP.nom:""} â Parametres</span>
        </div>
    ...
    <div style={{padding:16,fontFamily:"Georgia,serif"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <div>
          ...
          <div style={{fontSize:11,color:T.muted}}>Vue globale â {actifs.length} syndicat(s) actif(s)</div>
        </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:16}}>
            {actifs.map(function(s){return(
              <div key={s.id} style={{background:T.surface,border:"1px solid "+T.border,borderRadius:10,padding:14}}>
                ...
                <div style={{fontSize:11,color:T.muted,marginBottom:8}}>Dernier rapport: {s.dernierRapport||"â"}</div>
