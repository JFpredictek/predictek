...
// ===== EXERCICE FINANCIER SYNDICAT =====
var EXERCICES_SYND_INIT=[
  {id:1,code:"EX-2024-2025",debut:"2024-11-01",fin:"2025-10-31",statut:"ferme",dateClot:"2025-11-30",approuvePar:"CA Piedmont",notes:"Ferme et approuve en AGO du 1er dec 2025."},
  {id:2,code:"EX-2025-2026",debut:"2025-11-01",fin:"2026-10-31",statut:"ouvert",dateClot:"",approuvePar:"",notes:"Exercice en cours."},
];
var PERIODES_SYND_INIT=[
  {mois:"2025-11",nom:"Novembre 2025",statut:"ferme",dateClot:"2025-12-05",closePar:"Jean-Francois Laroche"},
  {mois:"2025-12",nom:"Decembre 2025",statut:"ferme",dateClot:"2026-01-07",closePar:"Jean-Francois Laroche"},
  {mois:"2026-01",nom:"Janvier 2026",statut:"ferme",dateClot:"2026-02-04",closePar:"Jean-Francois Laroche"},
  {mois:"2026-02",nom:"Fevrier 2026",statut:"ferme",dateClot:"2026-03-05",closePar:"Jean-Francois Laroche"},
  {mois:"2026-03",nom:"Mars 2026",statut:"ferme",dateClot:"2026-04-03",closePar:"Jean-Francois Laroche"},
...
var BUDGETS_SYND_INIT=[
  {id:1,titre:"Budget syndicat 2025-2026",exercice:"EX-2025-2026",version:"v1.0",statut:"approuve",creePar:"Jean-Francois Laroche",dateCree:"2025-10-15",approuvePar:"CA Piedmont",dateApprob:"2025-10-25",totalRevenus:171429,totalCharges:154800,surplus:16629,notes:"Approuve a l unanimite en reunion CA du 25 oct 2025."},
  {id:2,titre:"Budget revisite — hausses 2026",exercice:"EX-2025-2026",version:"v1.1",statut:"soumis",creePar:"Jean-Francois Laroche",dateCree:"2026-04-01",approuvePar:"",dateApprob:"",totalRevenus:171429,totalCharges:158200,surplus:13229,notes:"Revision suite a hausse assurances et deneigement."},
];
function TabExercicesSynd(){
  var s0=useState("exercices");var ong=s0[0];var setOng=s0[1];
  var s1=useState(EXERCICES_SYND_INIT);var exercices=s1[0];var setExercices=s1[1];
  var s2=useState(PERIODES_SYND_INIT);var periodes=s2[0];var setPeriodes=s2[1];
  var s3=useState(BUDGETS_SYND_INIT);var budgets=s3[0];var setBudgets=s3[1];
  var s4=useState(false);var showClotEx=s4[0];var setShowClotEx=s4[1];
  var s5=useState(false);var showClotPer=s5[0];var setShowClotPer=s5[1];
  var s6=useState(null);var selPer=s6[0];var setSelPer=s6[1];
  var s7=useState(false);var showBudWf=s7[0];var setShowBudWf=s7[1];
  var s8=useState(null);var selBud=s8[0];var setSelBud=s8[1];
  var s9=useState("");var wfPar=s9[0];var setWfPar=s9[1];
  var s10=useState("");var wfNotes=s10[0];var setWfNotes=s10[1];
  var s11=useState("");var clotPar=s11[0];var setClotPar=s11[1];
  ...
  var budEnAttente=budgets.filter(function(b){return b.statut==="soumis";}).length;

  var EX_ST={ouvert:{c:T.accent,bg:T.accentL,l:"Ouvert"},ferme:{c:T.muted,bg:T.alt,l:"Ferme"}};
  var PER_ST={ferme:{c:T.muted,bg:T.alt,l:"Fermee"},ouvert:{c:T.accent,bg:T.accentL,l:"Ouverte"},futur:{c:T.blue,bg:T.blueL,l:"A venir"}};
  var BUD_ST={brouillon:{c:T.muted,bg:T.alt,l:"Brouillon",step:1},soumis:{c:T.amber,bg:T.amberL,l:"Soumis",step:2},approuve:{c:T.accent,bg:T.accentL,l:"Approuve",step:3},ferme:{c:T.navy,bg:T.blueL,l:"Ferme",step:4}};
  var STEPS=["Brouillon","Soumis","Approuve","Ferme"];

  function fermerExercice(){
    setExercices(function(prev){return prev.map(function(e){return e.statut==="ouvert"?Object.assign({},e,{statut:"ferme",dateClot:today(),approuvePar:clotPar,notes:clotPar+" — "+today()}):e;});});
    setShowClotEx(false);setClotPar("");
  }
  function fermerPeriode(){
    setPeriodes(function(prev){return prev.map(function(x){return x.mois===selPer?Object.assign({},x,{statut:"ferme",dateClot:today(),closePar:clotPar}):x;});});
    setShowClotPer(false);setClotPar("");setSelPer(null);
  }
  function avancerBudget(action){
    setBudgets(function(prev){return prev.map(function(b){
      if(b.id!==selBud)return b;
      if(action==="approuver")return Object.assign({},b,{statut:"approuve",approuvePar:wfPar,dateApprob:today(),notes:wfNotes||b.notes});
      if(action==="rejeter")return Object.assign({},b,{statut:"brouillon",notes:(b.notes||"")+" [Rejete "+today()+"]"});
      if(action==="fermer")return Object.assign({},b,{statut:"ferme"});
      return b;
    });});
    setShowBudWf(false);setWfPar("");setWfNotes("");setSelBud(null);
  }

  var INNER_TABS=[
    {id:"exercices",l:"Exercices"},
    {id:"periodes",l:"Periodes"},
    {id:"budgets",l:"Budgets"+(budEnAttente>0?" ("+budEnAttente+")":"")},
  ];

  return(
    <div>
      <div style={{display:"flex",gap:3,marginBottom:14,background:T.surface,padding:4,borderRadius:9,border:"1px solid "+T.border}}>
        {INNER_TABS.map(function(t){var a=ong===t.id;return(
          <button key={t.id} onClick={function(){setOng(t.id);}} style={{background:a?T.navy:"transparent",border:"none",borderRadius:7,padding:"6px 14px",color:a?"#fff":T.muted,fontSize:11,cursor:"pointer",fontFamily:"inherit",fontWeight:a?600:400}}>{t.l}</button>
        );})}
        <div style={{marginLeft:"auto",display:"flex",gap:8,alignItems:"center"}}>
          {exCourant&&<Bdg bg={T.accentL} c={T.accent}>{exCourant.code}</Bdg>}
          {periodeOuverte&&<Bdg bg={T.amberL} c={T.amber}>{periodeOuverte.nom}</Bdg>}
        </div>
      </div>
      ...
      {ong==="exercices"&&(
        <div>
          {exCourant&&(
            <div style={{background:"linear-gradient(135deg,"+T.navy+",#1e3a5f)",borderRadius:12,padding:18,marginBottom:14,color:"#fff"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div>
                  ...
                  <div style={{fontSize:18,fontWeight:800}}>{exCourant.code}</div>
                  <div style={{fontSize:12,color:"#8da0bb",marginTop:3}}>{exCourant.debut} au {exCourant.fin}</div>
                </div>
                <div style={{textAlign:"right"}}>
                  <Bdg bg={"#ffffff20"} c={"#3CAF6E"}>Ouvert</Bdg>
                  <div style={{fontSize:11,color:"#8da0bb",marginTop:8}}>Mois courus:</div>
                  <div style={{fontSize:18,fontWeight:800,color:"#3CAF6E"}}>{Math.max(0,Math.ceil((new Date()-new Date(exCourant.debut))/2592000000))} / 12</div>
                </div>
              </div>
              <div style={{marginTop:14,paddingTop:12,borderTop:"1px solid #ffffff20"}}>
                <Btn sm bg={"#B83232"} onClick={function(){setShowClotEx(true);}}>Fermer et cloturer l exercice</Btn>
              </div>
            </div>
          )}
          {exercices.map(function(ex){var st=EX_ST[ex.statut]||EX_ST.ferme;return(
            <div key={ex.id} style={{background:T.surface,border:"1px solid "+(ex.statut==="ouvert"?T.accent:T.border),borderRadius:10,padding:14,marginBottom:8}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                <div><div style={{fontSize:13,fontWeight:700,color:T.navy}}>{ex.code}</div><div style={{fontSize:11,color:T.muted}}>{ex.debut} — {ex.fin}</div></div>
                <Bdg bg={st.bg} c={st.c}>{st.l}</Bdg>
              </div>
              {ex.statut==="ferme"&&<div style={{fontSize:11,color:T.muted}}>Cloture: {ex.dateClot} | Approuve par: {ex.approuvePar}</div>}
              {ex.notes&&<div style={{marginTop:6,fontSize:11,color:T.muted,background:T.alt,borderRadius:6,padding:"6px 10px"}}>{ex.notes}</div>}
            </div>
          );})}
          <Modal show={showClotEx} onClose={function(){setShowClotEx(false);}} title="Cloturer l exercice financier" w={460}>
            <div style={{background:T.redL,borderRadius:9,padding:"11px 14px",marginBottom:14,fontSize:12,color:T.red}}>
              La fermeture bloque toutes les ecritures et reporte les soldes au prochain exercice. Action irreversible.
            </div>
            <div style={{marginBottom:12}}><Lbl l="Cloture approuvee par"/><input value={clotPar} onChange={function(e){setClotPar(e.target.value);}} placeholder="President du CA" style={INP}/></div>
            <div style={{display:"flex",gap:8}}>
              <Btn bg={T.red} dis={!clotPar} onClick={fermerExercice}>Confirmer la cloture</Btn>
              <Btn onClick={function(){setShowClotEx(false);}} bg={T.alt} tc={T.muted} bdr={"1px solid "+T.border}>Annuler</Btn>
            </div>
          </Modal>
        </div>
      )}
      ...
      {ong==="periodes"&&(
        <div>
          <div style={{background:T.surface,border:"1px solid "+T.border,borderRadius:10,overflow:"hidden",marginBottom:14}}>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr style={{background:T.navy}}>{["Periode","Statut","Date cloture","Responsable","Action"].map(function(h){return <th key={h} style={{padding:"8px 12px",fontSize:9,fontWeight:700,color:"#8da0bb",textAlign:"left"}}>{h}</th>;})}</tr></thead>
              <tbody>
                {periodes.map(function(per){var st=PER_ST[per.statut]||PER_ST.futur;return(
                  <tr key={per.mois} style={{borderBottom:"1px solid "+T.border,background:per.statut==="ouvert"?"#FFFBF0":T.surface}}>
                    <td style={{padding:"10px 12px",fontWeight:600,fontSize:13}}>{per.nom}</td>
                    <td style={{padding:"10px 12px"}}><Bdg bg={st.bg} c={st.c}>{st.l}</Bdg></td>
                    <td style={{padding:"10px 12px",fontSize:11,color:T.muted}}>{per.dateClot||"—"}</td>
                    <td style={{padding:"10px 12px",fontSize:11,color:T.muted}}>{per.closePar||"—"}</td>
                    <td style={{padding:"10px 12px"}}>
                      {per.statut==="ouvert"&&<Btn sm bg={T.amber} onClick={function(){setSelPer(per.mois);setShowClotPer(true);}}>Fermer</Btn>}
                    </td>
                  </tr>
                );})}
              </tbody>
          ...
          <div style={{background:T.blueL,borderRadius:8,padding:"9px 14px",fontSize:11,color:T.blue}}>Une periode fermee ne peut plus recevoir de nouvelles ecritures.</div>
          <Modal show={showClotPer} onClose={function(){setShowClotPer(false);}} title="Fermer la periode" w={440}>
            <div style={{background:T.amberL,borderRadius:9,padding:"10px 14px",marginBottom:12,fontSize:11,color:T.amber}}>Assurez-vous que toutes les ecritures du mois sont completees avant de fermer.</div>
            <div style={{marginBottom:12}}><Lbl l="Ferme par"/><input value={clotPar} onChange={function(e){setClotPar(e.target.value);}} placeholder="Votre nom" style={INP}/></div>
            <div style={{display:"flex",gap:8}}>
              <Btn bg={T.amber} dis={!clotPar} onClick={fermerPeriode}>Confirmer</Btn>
              <Btn onClick={function(){setShowClotPer(false);}} bg={T.alt} tc={T.muted} bdr={"1px solid "+T.border}>Annuler</Btn>
            </div>
          </Modal>
        </div>
      )}
      ...
      {ong==="budgets"&&(
        <div>
          {budgets.map(function(b){var st=BUD_ST[b.statut]||BUD_ST.brouillon;return(
            <div key={b.id} style={{background:T.surface,border:"1px solid "+(b.statut==="soumis"?T.amber:T.border),borderRadius:10,padding:16,marginBottom:10}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
                <div><div style={{fontSize:13,fontWeight:700,color:T.navy}}>{b.titre}</div><div style={{fontSize:11,color:T.muted}}>{b.exercice} | {b.version} | Cree: {b.dateCree}</div></div>
                <Bdg bg={st.bg} c={st.c}>{st.l}</Bdg>
              </div>
              <div style={{display:"flex",gap:0,marginBottom:10}}>
                {STEPS.map(function(step,i){var done=st.step>i+1;var current=st.step===i+1;return(
                  <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",position:"relative"}}>
                    <div style={{width:20,height:20,borderRadius:"50%",background:done?T.accent:current?T.amber:T.border,display:"flex",alignItems:"center",justifyContent:"center",zIndex:1}}>
                      <span style={{fontSize:8,fontWeight:800,color:"#fff"}}>{done?"V":i+1}</span>
                    </div>
                    {i<3&&<div style={{position:"absolute",top:9,left:"50%",width:"100%",height:2,background:done?T.accent:T.border,zIndex:0}}/>}
                    <div style={{fontSize:8,color:done?T.accent:current?T.amber:T.muted,marginTop:3,textAlign:"center",fontWeight:current?700:400}}>{step}</div>
                  </div>
                );})}
              </div>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:T.muted,marginBottom:8}}>
                <span>Revenus: {fmt(b.totalRevenus)}</span>
                <span>Charges: {fmt(b.totalCharges)}</span>
                <span style={{color:b.surplus>=0?T.accent:T.red,fontWeight:600}}>Surplus: {fmt(b.surplus)}</span>
              </div>
              {b.approuvePar&&<div style={{fontSize:11,color:T.accent}}>Approuve par {b.approuvePar} le {b.dateApprob}</div>}
              {b.notes&&<div style={{fontSize:11,color:T.muted,marginTop:4,background:T.alt,borderRadius:6,padding:"5px 9px"}}>{b.notes}</div>}
              <div style={{marginTop:10,display:"flex",gap:6}}>
                {b.statut==="soumis"&&<Btn sm onClick={function(){setSelBud(b.id);setShowBudWf(true);}}>Approuver</Btn>}
                {b.statut==="soumis"&&<Btn sm bg={T.red} onClick={function(){setSelBud(b.id);avancerBudget("rejeter");}}>Rejeter</Btn>}
                {b.statut==="approuve"&&<Btn sm bg={T.navy} onClick={function(){setSelBud(b.id);avancerBudget("fermer");}}>Fermer le budget</Btn>}
              </div>
            </div>
          );})}
          <Modal show={showBudWf} onClose={function(){setShowBudWf(false);}} title="Approuver le budget" w={460}>
            <div style={{background:T.accentL,borderRadius:9,padding:"10px 14px",marginBottom:12,fontSize:12,color:T.accent}}>L approbation du CA officialise les montants. Le budget ne pourra plus etre modifie.</div>
            <div style={{marginBottom:10}}><Lbl l="Approuve par"/><input value={wfPar} onChange={function(e){setWfPar(e.target.value);}} placeholder="President ou responsable CA" style={INP}/></div>
            <div style={{marginBottom:14}}><Lbl l="Resolution CA / Notes"/><textarea value={wfNotes} onChange={function(e){setWfNotes(e.target.value);}} rows={3} style={Object.assign({},INP,{resize:"vertical"})}/></div>
            <div style={{display:"flex",gap:8}}>
              <Btn dis={!wfPar} onClick={function(){avancerBudget("approuver");}}>Approuver</Btn>
              <Btn onClick={function(){setShowBudWf(false);}} bg={T.alt} tc={T.muted} bdr={"1px solid "+T.border}>Annuler</Btn>
            </div>
          </Modal>
        </div>
      )}
...
export default function Gestionnaire(){
  ...
  var TABS=[{id:"bord",l:"Tableau de bord"},{id:"finances",l:"Finances"},{id:"reunions",l:"Reunions et PV"},{id:"carnet",l:"Carnet entretien"},{id:"unites",l:"Unites"},{id:"copros",l:"Coproprietaires"},{id:"fournisseurs",l:"Fournisseurs"},{id:"soldes",l:"Soldes ouv."},{id:"grandlivre",l:"Grand livre"},{id:"budget",l:"Budget"},{id:"exercices",l:"Exercices/Budgets"}];
  ...
  return(
    <div style={{padding:16,fontFamily:"Georgia,serif"}}>
      ...
      {ong==="exercices"&&<TabExercicesSynd/>}
