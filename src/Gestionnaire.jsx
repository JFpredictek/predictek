...
var money=function(n){if(!n&&n!==0)return"ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ";return(n<0?"-":"")+Math.abs(n).toLocaleString("fr-CA",{minimumFractionDigits:2,maximumFractionDigits:2})+" $";};
...
function Card(p){return <div style={Object.assign({background:T.surface,border:"1px solid "+T.border,borderRadius:12,padding:16},p.s||{})}>{p.children}</div>;}
function SH(p){return <div style={Object.assign({fontSize:10,color:T.muted,textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:8,fontWeight:600},p.s||{})}>{p.l}</div>;}
function Modal(p){
...
// COPROPRIÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂTAIRES pour export
const COPROS_EXPORT=[
...
function exportCSV(cols, data, filename) {
  ...
  var rows = data.map(function(row){
    return cols.map(function(c){
      var val = row[c.key] !== undefined ? String(row[c.key]) : "";
      var s = val.replace(new RegExp(q,"g"), q+q);
      return s.indexOf(",")>=0||s.indexOf(q)>=0||s.indexOf("\n")>=0 ? q+s+q : s;
    }).join(",");
  });
...
function ExportModal(p) {
  ...
  function doExport() {
    ...
    if(fmt==="csv") {
      exportCSV(selCols, data, "registre-copropriÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ©taires-piedmont-"+td());
    } else {
  ...
  return (
    <Modal open={p.open} onClose={p.onClose} title="Exporter la liste des coproprietaires" w={480}>
      ...
      <div style={{display:"flex",gap:8}}>
        <Btn onClick={doExport} fw={true}>Exporter {fmt==="csv"?"CSV":"PDF"}</Btn>
          ...
          <Card s={{marginBottom:12}}>
            ...
            {factEnAttente.map(function(f){return(
              <div key={f.id} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid "+T.border}}>
                <div><div style={{fontSize:12,fontWeight:600,color:T.text}}>{f.fournisseur}</div><div style={{fontSize:10,color:T.muted}}>{f.date} ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ {f.desc.substring(0,30)}</div></div>
      ...
      {sousOng==="budget"&&(
        <Card>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
            <b style={{fontSize:14,color:T.navy}}>Budget 2025-2026 ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ 6 mois</b>
            <div style={{display:"flex",gap:8}}>
      ...
      <div style={{width:300,flexShrink:0}}>
        ...
        {p.reunions.sort(function(a,b){return b.date.localeCompare(a.date);}).map(function(r){return(
          <div key={r.id} onClick={function(){setSel(r.id);}} style={{background:sel===r.id?T.accentLight:T.surface,border:"1px solid "+(sel===r.id?T.accent:T.border),borderRadius:10,padding:12,marginBottom:8,cursor:"pointer"}}>
            ...
            <div style={{fontSize:11,color:T.muted,marginTop:4}}>{r.ordre.length} pts{r.pv?" ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¢ PV disponible":""}</div>
          </div>
        );})}
      </div>
      <div style={{flex:1}}>
        ...
        {selR&&(
          <Card>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
              <div>
                <div style={{fontSize:16,fontWeight:800,color:T.navy}}>{selR.type} ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ {selR.date}</div>
...
function TabCarnet(p){
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <div>
          <b style={{fontSize:14,color:T.navy,display:"block"}}>Carnet d entretien ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ Loi 16</b>
          ...
          <tbody>
            {p.carnet.map(function(c){var st=statC(c);return(
              <tr key={c.id} style={{borderBottom:"1px solid "+T.border,background:st.pct>=80?st.bg:"transparent"}}>
                ...
                <td style={{padding:"9px 10px",color:T.muted,fontSize:11}}>{c.dernierEntretien||"ÃÂÃÂÃÂÃÂ¢ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ"}</td>
              </tr>
            );})}
          </tbody>
        </table>
...
export default function Gestionnaire(){
  ...
  var TABS=[
    {id:"bord",l:"Tableau de bord",icon:"ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ"},
    {id:"finances",l:"Finances",icon:"ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ°"},
    {id:"reunions",l:"Reunions & PV",icon:"ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ"},
    {id:"carnet",l:"Carnet entretien",icon:"ÃÂÃÂÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ"},
  ];
    ...
    <div style={{padding:16,fontFamily:"Georgia,serif"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          ...
          <Btn onClick={function(){setShowExport(true);}} bg={T.navy}>Liste copropriÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ©taires</Btn>
        </div>
      </div>
