...
export default function GestionFactures(){
  ...
  var seuilApprob=sel&&sel.approb_seuil!==null&&sel.approb_seuil!==undefined?parseFloat(sel.approb_seuil):0;
  function nbApprobPour(montant){
    var paliers=[];
    try{paliers=JSON.parse((sel&&sel.approb_paliers)||"[]");}catch(e){}
    if(!Array.isArray(paliers)||paliers.length===0)paliers=[{max:1000,nb:1},{max:5000,nb:2},{max:10000,nb:3}];
    paliers=paliers.slice().sort(function(a,b){return (a.max||0)-(b.max||0);});
    for(var i=0;i<paliers.length;i++){if(montant<=(parseFloat(paliers[i].max)||0))return Math.max(1,parseInt(paliers[i].nb)||1);}
    return Math.max(1,parseInt(sel&&sel.approb_nb_max)||3);
  }
