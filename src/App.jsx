  ...
  return(
    <div style={{padding:24,fontFamily:"Georgia,serif",maxWidth:700,margin:"0 auto"}}>
      ...
      <div style={{background:T.surface,border:"1px solid "+T.border,borderRadius:12,padding:24,marginBottom:16}}>
        ...
        <div style={{fontSize:12,color:T.muted,marginBottom:20}}>Le logo s affiche dans la barre de navigation. Formats: PNG, JPG, SVG. Taille recommandee: 200x200px.</div>
        <div style={{display:"flex",gap:24,alignItems:"flex-start"}}>
          <div style={{flexShrink:0}}>
            <div style={{fontSize:10,color:T.muted,fontWeight:600,textTransform:"uppercase",marginBottom:8}}>Apercu actuel</div>
          ...
          <div style={{flex:1}}>
            ...
            <div style={{display:"grid",gap:10}}>
              <button onClick={function(){document.getElementById("logoInputParams").click();}} style={{background:T.accent,color:"#fff",border:"none",borderRadius:8,padding:"12px 20px",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",textAlign:"left",display:"flex",alignItems:"center",gap:10}}>
                ...
                <div><div>Choisir un nouveau logo</div><div style={{fontSize:10,fontWeight:400,opacity:0.8}}>PNG, JPG ou SVG</div></div>
              </button>
              {p.logo&&<button onClick={resetLogo} style={{background:"#FDECEA",color:"#B83232",border:"1px solid #B83232",borderRadius:8,padding:"10px 20px",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>Retirer le logo — Revenir au P vert</button>}
            </div>
            {p.logo?<div style={{marginTop:12,background:T.accentL,borderRadius:8,padding:"10px 14px",fontSize:11,color:T.accent}}>Logo personnalise actif. Sauvegarde dans votre navigateur.</div>:<div style={{marginTop:12,background:T.blueL,borderRadius:8,padding:"10px 14px",fontSize:11,color:T.blue}}>Aucun logo personnalise. Cliquez ci-dessus pour ajouter le votre.</div>}
          </div>
        </div>
      </div>
      <div style={{background:T.surface,border:"1px solid "+T.border,borderRadius:12,padding:24,marginBottom:16}}>
        <div style={{fontSize:14,fontWeight:700,color:T.navy,marginBottom:16}}>Informations de la plateforme</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {[{l:"Version",v:"v7.0 — Avril 2026"},{l:"Syndicats actifs",v:"3"},{l:"URL",v:"predictek-d9sy.vercel.app"},{l:"Base de donnees",v:"Mode demo (Supabase a venir)"},{l:"Courriels",v:"Simulation (SendGrid a venir)"},{l:"SMS",v:"Simulation (Twilio a venir)"}].map(function(item,i){return(
      ...
      <div style={{background:"#FEF3E2",border:"1px solid #B8602044",borderRadius:12,padding:20}}>
        <div style={{fontSize:13,fontWeight:700,color:"#B86020",marginBottom:6}}>Prochaines fonctionnalites</div>
        <div style={{fontSize:12,color:"#B86020",lineHeight:1.8}}>Supabase | SendGrid | Twilio | Auth securisee | Theme personnalisable | IA integree</div>
      </div>
    </div>
  );
}
