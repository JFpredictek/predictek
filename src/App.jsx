...
var MODS=[
  {id:"hub",label:"Administration",icon:"H",desc:"Syndicats, usagers, roles"},
  {id:"crm",label:"CRM Support",icon:"C",desc:"Tickets, IA, bons de travail"},
  {id:"fournisseurs",label:"Fournisseurs",icon:"F",desc:"Entretien, appels d offres"},
  {id:"gestionnaire",label:"Portail Gestionnaire",icon:"G",desc:"Finances, CA, carnet entretien"},
        ...
        <div style={{display:"flex",height:"100%"}}>
          {MODS.map(function(m){var a=active===m.id;return(
            <button key={m.id} onClick={function(){setActive(m.id);}} style={{display:"flex",alignItems:"center",gap:8,padding:"0 16px",height:"100%",background:a?"#ffffff12":"none",border:"none",borderBottom:a?"2px solid #3CAF6E":"2px solid transparent",cursor:"pointer",fontFamily:"Georgia,serif",whiteSpace:"nowrap"}}>
              <div style={{width:24,height:24,borderRadius:6,background:a?"#3CAF6E":"#ffffff18",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:a?"#fff":"#8da0bb",flexShrink:0}}>{m.icon}</div>
