export default async function handler(req, res) {
  ...
  try{
    ...
    if(docs.length===0){return res.status(400).json({error:"No docs"});}
    ...
    if(data.error){return res.status(500).json({error:data.error.message,type:data.error.type,full:data});}
    ...
    try{
      ...
      return res.status(200).json({ok:true,data:{},raw:txt});
    }
  }catch(e){
