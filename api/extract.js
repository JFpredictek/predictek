...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Verifier la taille totale des PDFs
    var totalSize = docs.reduce(function(acc, d){ return acc + (d.b64 ? d.b64.length : 0); }, 0);
    if(totalSize > 10000000) {
      return res.status(400).json({error:"PDF trop volumineux. Essayez avec un fichier de moins de 7MB."});
    }
    ...
    catch(e) { return res.status(500).json({error:"Reponse invalide Anthropic: "+raw.substring(0,200)}); }
    ...
    try {
      ...
      return res.status(200).json({ok:true, data:{}, raw:txt.substring(0,200)});
    }
